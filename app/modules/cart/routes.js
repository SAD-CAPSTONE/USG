...
router.delete('/list', (req, res)=>{
  ...
  req.session.cart.forEach((data, i)=>{
    data.inv == inv ? req.session.cart.splice(i, 1): 0;
  });
  res.send({cart: req.session.cart.length})
});
router.get('/list/subtotal', (req, res)=>{
  req.session.cart ? 0 : req.session.cart = [];
  let subtotal = req.session.cart ?
    req.session.cart.reduce((temp, obj)=>{
      return temp + (parseFloat(obj.curPrice) * obj.curQty);
    },0) : 0
