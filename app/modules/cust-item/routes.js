  ...
  *(tblproductlist)*(tblproductbrand)*(tblproductinventory)*(tblproductreview)*/
  db.query(`SELECT B.*, ROUND(AVG(Review.intStars),1)AS aveRating, COUNT(Review.intProductReviewNo)AS cntRating,
    COUNT(Review.strReview)AS cntReview FROM( SELECT A.*, Orders.intOrderDetailsNo, COUNT(Orders.intOrderDetailsNo)AS OrderCNT FROM(
		SELECT tblproductlist.*, Cat.intCategoryNo, Inv.intInventoryNo, Inv.intStatus As InvStatus, Inv.productPrice, Inv.intQuantity, Brand.strBrand, Stock FROM tblproductlist
		INNER JOIN (SELECT * FROM tblproductbrand)Brand ON tblproductlist.intBrandNo= Brand.intBrandNo
		INNER JOIN (SELECT * FROM tblproductinventory)Inv ON tblproductlist.intProductNo= Inv.intProductNo
		INNER JOIN (SELECT intInventoryNo, (SUM(intQuantity) - SUM(intReservedItems))Stock FROM tblbatch GROUP BY intInventoryNo)Batch
		ON Inv.intInventoryNo= Batch.intInventoryNo INNER JOIN (SELECT * FROM tblsubcategory)Cat ON tblproductlist.intSubCategoryNo= Cat.intSubCategoryNo
    ...
    WHERE B.intCategoryNo= 1000 AND B.intProductNo != 1005 GROUP BY B.intProductNo ORDER BY intSubCategoryNo!= 1010, OrderCNT DESC LIMIT 10`
    ...
    if (!results[0]){
      db.query(`SELECT A.*, ROUND(AVG(Review.intStars),1)AS aveRating, COUNT(Review.intProductReviewNo)AS cntRating, COUNT(Review.strReview)AS cntReview FROM(
        ...
        INNER JOIN (SELECT * FROM tblproductinventory)Inv ON tblproductlist.intProductNo= Inv.intProductNo
    		INNER JOIN (SELECT intInventoryNo, (SUM(intQuantity) - SUM(intReservedItems))Stock FROM tblbatch GROUP BY intInventoryNo)Batch
    		ON Inv.intInventoryNo= Batch.intInventoryNo WHERE Brand.intStatus= 1 AND Stock > 0 GROUP BY tblproductlist.intProductNo)A
        LEFT JOIN (SELECT * FROM tblproductreview)Review ON A.intProductNo = Review.intProductNo
        WHERE A.intProductNo != 1001 GROUP BY A.intProductNo LIMIT 10`
  ...
  *(tblproductlist)*(tblproductbrand)*(tblproductinventory)*(tblorderdetails)*(tblproductreview)*/
  db.query(`SELECT B.*, ROUND(AVG(Review.intStars),1)AS aveRating, COUNT(Review.intProductReviewNo)AS cntRating,
    COUNT(Review.strReview)AS cntReview FROM(SELECT A.*, Orders.intOrderDetailsNo, COUNT(Orders.intOrderDetailsNo)AS OrderCNT FROM(
		...
		INNER JOIN (SELECT * FROM tblproductinventory)Inv ON tblproductlist.intProductNo= Inv.intProductNo
    INNER JOIN (SELECT intInventoryNo, (SUM(intQuantity) - SUM(intReservedItems))Stock FROM tblbatch GROUP BY intInventoryNo)Batch
    ON Inv.intInventoryNo= Batch.intInventoryNo WHERE Brand.intStatus= 1 AND Stock > 0)A
  	LEFT JOIN (SELECT * FROM tblorderdetails)Orders ON A.intInventoryNo= Orders.intInventoryNo GROUP BY A.intProductNo)B
